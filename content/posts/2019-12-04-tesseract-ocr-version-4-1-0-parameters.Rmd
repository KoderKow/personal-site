...
## Tesseract Data
tp <- "editor_image_xpos,590,Editor image X Pos
...
editor_debug_config_file,NA,Config file to apply to single words
fx_debugfile,NA,FXDebug Name of debugfile
classify_font_name,UnknownFont,Default font name to be used in training
dotproduct,NA,generic Function used for calculation of dot product
debug_file,NA,File to send tprintf output to
...
classify_learning_debug_level,0,Learning Debug Level:,matcher_permanent_classes_min,1,Min # of permanent classes
...
classify_class_pruner_multiplier,15,Class Pruner Multiplier 0-255:,classify_cp_cutoff_strength,7,Class Pruner CutoffStrength:,classify_integer_matcher_multiplier,10,Integer Matcher Multiplier,0-255:,il1_adaption_test,0,Dont adapt to i/I at beginning of word
...
repair_unchopped_blobs,1,Fix blobs that arent chopped
...
chop_centered_maxwidth,90,Width of (smaller) chopped blobs above which we dont care that a chop is not near the center.
...
tessedit_pageseg_mode,6,Page seg mode: 0=osd only, 1=auto+osd, 2=auto_only, 3=auto, 4=column, 5=block_vert, 6=block, 7=line, 8=word, 9=word_circle, 10=char,11=sparse_text, 12=sparse_text+osd, 13=raw_line (Values from PageSegMode enum in publictypes.h)
...
lstm_choice_mode,0,Allows to include alternative symbols choices in the hOCR output. Valid input values are 0, 1, 2 and 3. 0 is the default value. With 1 the alternative symbol choices per timestep are included. With 2 the alternative symbol choices are accumulated per character.
...
segment_nonalphabetic_script,0,Dont use any alphabetic-specific tricks. Set to true in the traineddata config file for scripts that are cursive or inherently fixed-pitch
...
wordrec_no_block,0,Dont output block information
...
tosp_old_to_method,0,Space stats use prechopping?,tosp_old_to_constrain_sp_kn,0,Constrain relative values of inter and intra-word gaps for old_to_method.
tosp_only_use_prop_rows,1,Block stats to use fixed pitch rows?,tosp_force_wordbreak_on_punct,0,Force word breaks on punct to break long lines in non-space delimited langs
tosp_use_pre_chopping,0,Space stats use prechopping?,tosp_old_to_bug_fix,0,Fix suspected bug in old code
...
tosp_all_flips_fuzzy,0,Pass ANY flip to context?,tosp_fuzzy_limit_all,1,Dont restrict kn->sp fuzzy limit to tables
...
textord_no_rejects,0,Dont remove noise blobs
...
classify_learn_debug_str,NA,Class str to debug learning
user_words_file,NA,A filename of user-provided words.
user_words_suffix,NA,A suffix of user-provided words located in tessdata.
user_patterns_file,NA,A filename of user-provided patterns.
user_patterns_suffix,NA,A suffix of user-provided patterns located in tessdata.
output_ambig_words_file,NA,Output file for ambiguities found in the dictionary
word_to_debug,NA,Word for which stopper debug information should be printed to stdout
word_to_debug_lengths,NA,Lengths of unichars in word_to_debug
tessedit_char_blacklist,NA,Blacklist of chars not to recognize
tessedit_char_whitelist,NA,Whitelist of chars to recognize
tessedit_char_unblacklist,NA,List of chars to override tessedit_char_blacklist
tessedit_write_params_to_file,NA,Write all parameters to the given file.
...
tessedit_load_sublangs,NA,List of languages to load with this one
page_separator,NA,Page separator (default is form feed control character)
...
superscript_bettered_certainty,0.97,What reduction in badness do we think sufficient to choose a superscript over what wed thought.,For example, a value of 0.6 means we want to reduce badness of certainty by at least 40%
...
subscript_max_y_top,0.5,Maximum top of a character measured as a multiple of x-height above the baseline for us to reconsider whether its a subscript.
superscript_min_y_bottom,0.3,Minimum bottom of a character measured as a multiple of x-height above the baseline for us to reconsider whether its a superscript.
...
tesseract_parameters <- vroom::vroom(
  file = tp,
  delim = ",",
  col_names = FALSE,
  .name_repair = ~ c("parameter", "value", "description")
) %>%
  arrange(parameter)

## Fix values that have single and double quotes
tesseract_parameters$value[32] <- "(\u0027`\u0022"
tesseract_parameters$value[33] <- ").,;:?!"
tesseract_parameters$value[34] <- "(\u0027`\u0022"
tesseract_parameters$value[236] <- "ij!?%\u0022:;"

## JSON output to share
tp_json <- jsonlite::toJSON(tesseract_parameters)
```

# Overview

During a project my team was having a hard time tracking down an up to date parameters list for [Tesseract OCR](https://github.com/tesseract-ocr/tesseract). The documentation mentions to use `tesseract --print-parameters` to get the available *600+* options. Personally, I use R at my job so sifting through the terminal and trying to go through a poorly formatted (this may be due to my unfamiliarity to the terminal) list in the terminal can be a headache. I have taken the current parameters list for **Tesseract OCR 4.1.0** and made it available on this page and in JSON format in hopes it will help people explore the available options.

<!-- Below the table there will will be this table is JSON for those wanting the raw data. -->

**Disclaimer:** The *Description* column has apostrophe's **removed** due to the major conflicts it was causing me during data cleaning. There are some single and double quotes in the *Value* column, these were intially removed and then re-added. I have triple-checked to ensure they are correct since *Value* is important when looking at the default value of these parameters.
